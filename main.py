...
for filepath in filepaths:
    ...
    pdf.cell(w=33, h=8, txt=columns[3], border=1)
    pdf.cell(w=33, h=8, txt=columns[4], border=1, ln=1)
    ...
    for index, row in df.iterrows():
        ...
        pdf.cell(w=33, h=8, txt=str(row["total_price"]), border=1, ln=1)
    ...
    pdf.cell(w=33, h=8, txt="", border=1)
    pdf.cell(w=33, h=8, txt=str(total_sum), border=1, ln=1)

    # Add total sum sentence
    pdf.ln(5)
    pdf.set_font(family="Times", size=12, style="B")
    pdf.set_text_color(80, 80, 80)
    pdf.cell(w=33, h=8, txt=f"The total price is {total_sum}.", ln=1)

    # Add company name and logo
    pdf.set_font(family="Times", size=16, style="B")
    pdf.set_text_color(80, 80, 80)
    pdf.cell(w=31, h=8, txt=f"PythonHow")
    pdf.image("images/Pythonhow.png", w=7)
